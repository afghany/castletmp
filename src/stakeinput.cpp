...
bool CCstlStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CCstlStake from zerocoin spend", __func__);
...
bool CCstlStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CCstlStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CCstlStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CCstlStake::GetValue() const
{
...
CDataStream CCstlStake::GetUniqueness() const
{
...
bool CCstlStake::ContextCheck(int nHeight, uint32_t nTime)
{
