...
extern UniValue DoZcstlSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setzcstlseed(const JSONRPCRequest& request);
extern UniValue getzcstlseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdzcstl(const JSONRPCRequest& request);
extern UniValue dzcstlstate(const JSONRPCRequest& request);
