...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total castle balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zCSTLsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCSTL denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCSTL denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCSTL denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCSTL denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCSTL denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCSTL denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCSTL denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCSTL denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCSTL denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in castle/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in castle/kb\n"
    ...
    UniValue zcstlObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zcstlObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zcstlObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcstlObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zCSTLsupply", zcstlObj));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"castleaddress\"\n"
            "\nReturn information about the given castle address.\n"

            "\nArguments:\n"
            "1. \"castleaddress\"     (string, required) The castle address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"castleaddress\",    (string) The castle address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for CASTLE cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: CASTLE address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are castle addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) castle address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"castleaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"castleaddress\"  (string, required) The castle address to use for the signature.\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in castle.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in castle.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) CSTL value of the stakeable coins (minus reserve balance, if any)\n"
