// Copyright (c) 2018-2020 The DIVX developers
                    ...
                    if (tx.HasZerocoinSpendInputs()) {
                        for (auto& in : tx.vin) {
                            ...
                            if (isPublicSpend) {
                                ...
                                CValidationState state;
                                if (!ZCSTLModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
{
    CDataStream serializedCoinSpend = ZCSTLModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZCSTLSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    //if (pindex->nHeight < 500000)
   //    return true;
    if (!consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC))
        return true;

    //Add mints to zCSTL supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zCSTL supply
    ...
    // A one-time event where only the zCSTL supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZCSTLSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    ...
    // Re-add spends to zCSTL supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zCSTL supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
