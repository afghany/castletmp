...
class ReorgStakeTest(CastleTestFramework):
    ...
    def check_money_supply(self, expected_cstl, expected_zcstl):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected CSTL and zCSTL supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_cstl))
            for denom in node['zCSTLsupply']:
                assert_equal(node['zCSTLsupply'][denom], DecimalAmt(expected_zcstl[denom]))
    ...
    def run_test(self):
        ...
        # Check CSTL and zCSTL supply at the beginning
        # ------------------------------------------
        # zCSTL supply: 2 coins for each denomination
        expected_zcstl_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # CSTL supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zcstl_supply)
        ...
        # Verify that CSTL and zCSTL supplies were properly updated after the spends and reorgs
        self.log.info("Check CSTL and zCSTL supply...")
        ...
        expected_zcstl_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zcstl_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zcstl_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zcstl_supply)
