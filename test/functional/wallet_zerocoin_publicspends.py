...
class ZerocoinSpendTest(CastleTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zcstl_bal, cstl_bal):
            zcstl_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zcstl_bal)
            cstl_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], cstl_bal)
            return zcstl_bal, cstl_bal
        ...
        zcstl_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zcstl_balance, 6666)
        ...
        zcstl_balance, balance = check_balances(denom_2, zcstl_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zcstl_balance, balance = check_balances(denom_3, zcstl_balance, balance)
        ...
        _, _ = check_balances(denom_1, zcstl_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
